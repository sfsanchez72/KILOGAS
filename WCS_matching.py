...
##########################################################################
hdu = fits.open('/mnt/naschiripa/KILOGAS/Pipe3D/tables/KG_Pipe3D_table_COSMO.fits')
...
output_dir = '/mnt/naschiripa/KILOGAS/Pipe3D/PanStarss'
os.makedirs(output_dir, exist_ok=True)

# Loop through the rows in tab_KG
for row in tab_KG:
    cube_file = f"/mnt/naschiripa/KILOGAS/Pipe3D/dataproducts/{row['name']}.Pipe3D.cube.fits.gz"
    ...
    if os.path.exists(cube_file):  # Check if the datacube file exists
        ...
        try:
            if (os.path.exists(outpat_path)):
                print(f'file {output_path} already download')
            else:
...
for row in tab_KG:
    ...
    if os.path.exists(cube_file) and os.path.exists(g_band_file):  # Check if both files exist
        try:
            ...
            radius = np.sqrt((x - bc_cube_img[0])**2 + (y - bc_cube_img[1])**2)
            max_radius = np.min(center)
            distance_mask = radius < 8/0.25
            ...
            plt.savefig(f"/mnt/naschiripa/KILOGAS/Pipe3D/PS_img/{target_name}_KG.png", transparent=False, facecolor='white', edgecolor='white')#.pdf")
            #plt.show()
        except Exception as e:
