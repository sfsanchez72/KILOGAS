...
--panstarrs-dir Optional directory containing the Pan-STARRS g-band image
...
<panstarrs-dir>/<name>_g.fits, when --panstarrs-dir is provided
<input-dir>/PanStarss/<name>_g.fits
<input-dir>/PanSTARRS/<name>_g.fits
...
def get_background_image(
    ...
    name: str,
    fallback: np.ndarray,
    panstarrs_dir: Path | None = None,
) -> tuple[np.ndarray, str]:
    """Read optional Pan-STARRS g image; otherwise use a fallback Pipe3D map."""
    from astropy.io import fits
    candidates: list[Path] = []
    if panstarrs_dir is not None:
        candidates.extend([
            panstarrs_dir / f"{name}_g.fits",
            panstarrs_dir / f"{name}.fits",
        ])
    candidates.extend([
...
def make_figure(
    ...
    try:
        ...
        bg, bg_label = get_background_image(input_dir, name, fallback=ssp_data[0, :, :], panstarrs_dir=panstarrs_dir)
        ...
        out_path = output_dir / f"{name}_diag.png"
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "Optional directory containing the Pan-STARRS image. "
            "The script will search for <name>_g.fits and <name>.fits there before "
            "falling back to locations inside --input-dir."
        ),
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        make_figure(
            ...
            args.input_dir,
            args.output_dir,
            panstarrs_dir=args.panstarrs_dir,
            usetex=args.usetex,
        )
